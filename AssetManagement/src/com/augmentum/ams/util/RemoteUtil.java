...
public final class RemoteUtil {
     ...
     */
    public static Request getRequest(HttpServletRequest httpServletRequest, String key,
            String roleName, final String responseType, Object... other) {

        // Gain the url with the ticket
        String ticketUrl = RequestHelper.getTicketURLByKey(httpServletRequest, key, other);

        Request request = null;
        try {
            // Gain the request that used to communicate with IAP
            request = RequestHelper.getRequest(ticketUrl, key, roleName, responseType);

        } catch (NoAppNameException e) {
            throw new RemoteException(ErrorCodeUtil.DATA_IAP_0204001,
                    "Error from getting data from IAP!");
        }
        return request;
    }
    ...
    public static IAPDataResponseModel getResponse(IAPDataSearchModel model,
        ...
        String requestBody = IAPDataModelRequestUtil.buildSearchString(requestType, model);
        ...
        try {
            ...
            responseBody = RequestHelper.sendRequest(request, requestBody, requestType);
        } catch (ClientProtocolException e) {
            throw new RemoteException(e, ErrorCodeUtil.DATA_IAP_0204001,
                    "Error from getting data from IAP!");
        } catch (IOException e) {
            throw new RemoteException(e, ErrorCodeUtil.DATA_IAP_0204001,
                    "Error from getting data from IAP!");
        }
        IAPDataResponseModel responseModel = IAPDataModelResponseUtil
                .getResponseModel(responseBody);

...
public class AssetDaoImpl extends BaseDaoImpl<Asset> implements AssetDao {
	...
	public Asset getAssetById(String id) {
		DetachedCriteria detachedCriteria = DetachedCriteria
				.forClass(Asset.class);
		detachedCriteria.createAlias("user", "user", JoinType.LEFT.ordinal());
		detachedCriteria.createAlias("project", "project",
				JoinType.LEFT.ordinal());
		detachedCriteria.createAlias("location", "location",
				JoinType.LEFT.ordinal());
		detachedCriteria.createAlias("customer", "customer",
				JoinType.LEFT.ordinal());
	...
	public List<ExportVo> findAssetsByIdsForExport(final String[] assetIds) {
		...
		List<ExportVo> list = (List<ExportVo>) getHibernateTemplate()
				.executeFind(new HibernateCallback() {
					public Object doInHibernate(Session session)
							throws HibernateException, SQLException {
						List<ExportVo> list = (List<ExportVo>) session
								.createSQLQuery(sql.toString())
								.setResultTransformer(
										Transformers
												.aliasToBean(ExportVo.class))
								.list();
	...
	/**
	 * get map<AssetTypeName, AssetCount> based on the result of execute hql
	 * (List)
	 * 
	...
	public Map<String, Integer> getAssetCountForManager(User user,
		...
		// TODO judge user and customers if is null
		...
		query = session.createQuery(hql).setParameterList("customerIds",
				customerIds);
		map.put("allCustomersAssetTotal",
				Integer.valueOf(query.list().get(0).toString()));
		...
		for (Customer customer : customers) {
			...
			map.put(customer.getCustomerName() + "AssetTotal",
					getCountByHql(hql, customer.getId()));

			hql = "SELECT type, COUNT(*) FROM Asset WHERE isExpired = false AND customer.id = ? GROUP By type ORDER BY type";
			result = getAssetTypeCountList(getHibernateTemplate().find(hql,
					customer.getId()));

			map.put(customer.getCustomerName() + "SoftwareCount",
					result.get("SOFTWARE"));
			map.put(customer.getCustomerName() + "DeviceCount",
					result.get("DEVICE"));
			map.put(customer.getCustomerName() + "MachineCount",
					result.get("MACHINE"));
			map.put(customer.getCustomerName() + "MonitorCount",
					result.get("MONITOR"));
			map.put(customer.getCustomerName() + "OtherAssetsCount",
					result.get("OTHERASSETS"));
		}
		...
		query = session.createQuery(hql).setParameterList("customerIds",
				customerIds);
		map.put("allCustomersAvailableAssetTotal",
				Integer.valueOf(query.list().get(0).toString()));
		...
		map.put("allCustomersAvailableOtherAssetsCount",
				result.get("OTHERASSETS"));
		...
		for (Customer customer : customers) {
			...
			map.put(customer.getCustomerName() + "AvailableAssetTotal",
					getCountByHql(hql, customer.getId()));
			...
			map.put(customer.getCustomerName() + "AvailableSoftwareCount",
					result.get("SOFTWARE"));
			map.put(customer.getCustomerName() + "AvailableDeviceCount",
					result.get("DEVICE"));
			map.put(customer.getCustomerName() + "AvailableMachineCount",
					result.get("MACHINE"));
			map.put(customer.getCustomerName() + "AvailableMonitorCount",
					result.get("MONITOR"));
			map.put(customer.getCustomerName() + "AvailableOtherAssetsCount",
					result.get("OTHERASSETS"));
		}
		...
		query = session.createQuery(hql).setParameterList("customerIds",
				customerIds);
		map.put("allCustomersIdleAssetTotal",
				Integer.valueOf(query.list().get(0).toString()));
		...
		for (Customer customer : customers) {
			...
			hql = "SELECT COUNT(*) FROM Asset WHERE isExpired = false AND status = 'IDLE' AND customer.id = ?";
			map.put(customer.getCustomerName() + "IdleAssetTotal",
					getCountByHql(hql, customer.getId()));

			hql = "SELECT type, COUNT(*) FROM Asset WHERE isExpired = false AND status = 'IDLE' AND customer.id = ? GROUP BY type ORDER BY type";
			result = getAssetTypeCountList(getHibernateTemplate().find(hql,
					customer.getId()));

			map.put(customer.getCustomerName() + "IdleSoftwareCount",
					result.get("SOFTWARE"));
			map.put(customer.getCustomerName() + "IdleDeviceCount",
					result.get("DEVICE"));
			map.put(customer.getCustomerName() + "IdleMachineCount",
					result.get("MACHINE"));
			map.put(customer.getCustomerName() + "IdleMonitorCount",
					result.get("MONITOR"));
			map.put(customer.getCustomerName() + "IdleOtherAssetsCount",
					result.get("OTHERASSETS"));
		}
		...
		query = session.createQuery(hql).setParameterList("customerIds",
				customerIds);
		map.put("allCustomersInUseAssetTotal",
				Integer.valueOf(query.list().get(0).toString()));
		...
		for (Customer customer : customers) {
			...
			map.put(customer.getCustomerName() + "InUseAssetTotal",
					getCountByHql(hql, customer.getId()));
			...
			map.put(customer.getCustomerName() + "InUseSoftwareCount",
					result.get("SOFTWARE"));
			map.put(customer.getCustomerName() + "InUseDeviceCount",
					result.get("DEVICE"));
			map.put(customer.getCustomerName() + "InUseMachineCount",
					result.get("MACHINE"));
			map.put(customer.getCustomerName() + "InUseMonitorCount",
					result.get("MONITOR"));
			map.put(customer.getCustomerName() + "InUseOtherAssetsCount",
					result.get("OTHERASSETS"));
		}
	...
	public List<Asset> findIdleAssetForPanel(List<Customer> customers) {
		...
		criteria.setFetchMode("project", FetchMode.JOIN).setFetchMode("user",
				FetchMode.JOIN);

		criteria.createAlias("customer", "customer")
				.add(Restrictions.eq("isExpired", Boolean.FALSE))
				.add(Restrictions.eq("status", "IDLE"))
				.add(Restrictions.in("customer.id", customerIds))
				.addOrder(Order.desc("updatedTime"));

		return findByCriteria(criteria);
	}

	@Override
	public List<Asset> findWarrantyExpiredAssetForPanel() {

		DetachedCriteria criteria = DetachedCriteria.forClass(Asset.class);
		criteria.setFetchMode("customer", FetchMode.JOIN);

		criteria.add(Restrictions.eq("isExpired", Boolean.FALSE))
				.add(Restrictions.between("warrantyTime",

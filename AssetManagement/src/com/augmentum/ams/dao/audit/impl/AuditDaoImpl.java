...
public class AuditDaoImpl extends BaseDaoImpl<Audit> implements AuditDao {
	...
	@Override
	public List<Asset> findAuditedAssets(String auditFileName,
			int iDisplayStart, int iDisplayLength) {

		// String
		// hql="from Audit a where a.auditFile.fileName = ? and a.status = ? and a.isExpired = ?";
		...
		return getListForPage(hql, auditFileName, iDisplayStart,
				iDisplayLength, Boolean.TRUE);
	}

	@Override
	public List<Asset> findUnAuditedAssets(String auditFileName,
			int iDisplayStart, int iDisplayLength) {
	...
	private List<Asset> getListForPage(final String hql, final String fileName,
		...
		List<Asset> list = getHibernateTemplate().executeFind(
				new HibernateCallback() {
					public Object doInHibernate(Session session)
							throws HibernateException, SQLException {
						List<Asset> result = session.createQuery(hql)
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<String> findInventoryAssetId(Boolean isAudited, String auditFileName) {
		String hql = "SELECT asset.id FROM Audit WHERE isExpired = ? AND status = ? AND auditFile.fileName = ?";
		return getHibernateTemplate().find(hql, Boolean.FALSE, isAudited, auditFileName);
	}

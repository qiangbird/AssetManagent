...
//        this.criteria.sortName = 'createdTime';
        if(!this.opts.url){
    ...
    $.DataList.prototype = {
        init : function() {
                ...
                $(document).bind({
                    'mousedown.mike' : function(e){
                        if(!$.contains($('.dataList-div-pageSize',$self.$data)[0], e.target)){
                            ...
                            $('.dataList-a-pageSize').removeClass('dataList-a-pageSize-upimage');
                            $('.dataList-a-pageSize').addClass('dataList-a-pageSize-downimage');
            ...
            for(var i = 0; i < this.opts.columns.length; i++){
                ...
                fields.push('<div><a href="javascript:;" en="' + this.opts.columns[i].EN + '" index=' + i + ' class="' + clz + '" headerId="' + this.opts.columns[i].headerId + '"></a><span>' + column[this.opts.language] + '</span></div>');
            }
            ...
            $selectPageSize.css('margin-top', -1 * (this.opts.pageSizes.length * 24 + 2) );
            ...
            $pageSize.append($('<a>',{'class':'dataList-a-pageSize dataList-a-pageSize-downimage w40-h24','href':'javascript:;'}).text(this.criteria.pageSize));
            ...
            setTimeout(function(){
                ...
                $('.dataList-a-field-cancel',$self.$data).click(function(){
                    ...
                    for(var i = 0 ; i < selectColumns.length ; i++){
                        //English name has space need to change the space to '-'
                        $('.checkbox-' + selectColumns[i].replace(/\s/g, '-')).addClass('dataList-checkbox-active');
                    }
                     ...
                     // Get all checkbox value and related headerId value
                        $('.dataList-a-field-checkbox,.dataList-a-field-checkbox-active-disable', $self.$data).each(function(i){
                            if(!$(this).hasClass('dataList-a-field-checkbox_all')) {
                                customization.push(($(this).attr('headerId')));
                                if($(this).hasClass('dataList-checkbox-active') || $(this).hasClass('dataList-a-field-checkbox-active-disable')) {
                                    showDefault.push(true);
                                } else {
                                    showDefault.push(false);
                                }
                            }
                        });
                ...
                $('.dataList-a-pageSize',$self.$data).click(function(){
                    var prev = $(this).prev();
                    if(prev.is(':visible')){
                        $(this).removeClass('dataList-a-pageSize-upimage');
                        $(this).addClass('dataList-a-pageSize-downimage');
                        prev.slideUp();
                    }else{
                        $(this).removeClass('dataList-a-pageSize-downimage');
                        $(this).addClass('dataList-a-pageSize-upimage');
                        prev.slideDown();
                    }
                });
                $('.dataList-div-pageSize > div',$self.$data).click(function(){
                    $('.dataList-a-pageSize').prev().slideUp("fast");
                    ...
                    if($self.opts.updateShowSize.url && $self.opts.updateShowSize.callback){
                        ...
                        data['categoryFlag'] = parseInt($("#categoryFlag").val());
                        $.ajax({
        ...
        refreshColumn : function(){
            ...
            if(this.opts.hasOnlineCourseIcon){
                heads.push('<div class="w-30"></div>');
                _width += 30;
            }
...
//                var pointer = this.opts.columns[i].sortName ? 'pointer' : 'default';
                if(this.opts.columns[i].isMustShow || this.opts.columns[i].isShow){
                    ...
                    if(this.opts.columns[i].autoWidth){
                        ...
                        $('.' + this.opts.columns[i].EN.replace(/\s/g,'-'), this.$data).width(this.opts.columns[i].width + $self.params.avg);
                    }else{
        ...
        refresh : function(data) {
            ...
            for(var i = 0; i < data.fieldsData.length; i++){
                ...
                if(this.opts.hasOnlineCourseIcon){
                    var attachmentIconDiv = $("<div class='dataList-onlinecourse-icon w-30'></div>");
                    if (data.fieldsData[i][0 + increment] !== "") {
                        var attachment = $("<img class='onlineCourseImag' src= '" + $("#basePath").val() + "searchList/images/ICN_VideoCourse_16x18.png'/>");
                        attachment.attr("onlineCourseId", data.fieldsData[i][0 + increment]);
                        attachment.attr("courseId", data.fieldsData[i][0]);
                        attachment.attr("planId", data.fieldsData[i][0]);
                        attachmentIconDiv.append(attachment);
                    }
                    bodys.push(attachmentIconDiv.get(0).outerHTML);
                }
                if(this.opts.hasAttachmentIcon){
                    increment++;
                }
                if(this.opts.hasAttachmentIcon){
                    var attachmentIconDiv = $("<div class='dataList-attachment-icon w-30'></div>");
                    if (data.fieldsData[i][0 + increment] !== "") {
                        var attachment = $("<img class='downloadImag' src= '" + $("#basePath").val() + "searchList/images/ICN_Attachment_16x16.png'/>");
                        attachment.attr("attachmentNames", data.fieldsData[i][0 + increment]);
                        if (data.listType === "course") {
                            attachment.attr("courseId", data.fieldsData[i][0]);
                        } else {
                            attachment.attr("planId", data.fieldsData[i][0]);
                        }
            ...
            $self.params.$tips.poshytip({
                ...
                content : function(){
                    ...
                    $self.$heightTool.text($(this).text());
                        if($self.$heightTool.height() > 120 ){
                            title = '<div class="dataList-div-content">' +  $(this).text().replace(/</g,"&lt;").replace(/>/g,"&gt;") + '</div><div><a class="dataList-a-viewAll" href="javascript:;" onclick="$.DataList.viewAll(this);">[' + I18N[$self.opts.language].viewAll + ']<a></div>' ; 
                        }else{
                            title = '<div class="dataList-div-content">' +  $(this).text().replace(/</g,"&lt;").replace(/>/g,"&gt;") + '</div>' ; 
                        }
                        return title;
...
//                    }
            ...
            $(".downloadImag").each(function(index, item){
                var attachmentNames = $(item).attr("attachmentNames");
                var courseId = $(item).attr("courseId");
                var expr = "img[attachmentNames='" + attachmentNames +"']";
                $(expr).poshytip('destory');
                $(expr).poshytip({
                    allowTipHover : true ,
                    liveEvents: true,
                    className: 'tip-green',
                    content: attachmentNames.replace(/,/g,'<br>')
                });
            });
        },
        setShow : function(str){
            ...
            for(var i = 0 ; i < a.length ; i++){
              ...
              //If userCustomColumn.showDefault is true, isShow = true, column checkbox will be actived
                if(this.opts.columns[i].showDefault == true){
    ...
    $.DataList.viewAll = function(self){
        ...
        _self.height("auto");
        _self.addClass("dataList-div-content-voerflow");

	...
	* Add a item, argument :string
    ...
    function add_string_items(string) {
        ...
        if (string && string.indexOf(settings.inputSplit) != -1) {
            ...
            for ( var i in array) {
                var label = $.trim(array[i]);
                var value = $.map(settings.local_data, function(key) {
                    if (label == key.label) {
                        return key.value;
                    }
                });
...
//Add a token to the token list based on user input
      ...
      if(token_count > 0 && settings.preventDuplicates) {
          ...
          token_list.children().each(function () {
              ...
              if(existing_data && existing_data.label === item.label) {
                  //In OT system when the employee name is the same, The job Id maybe not the same
                  if(existing_data.value === item.value) {
                      found_existing_token = existing_token;
                      return false;
                  } else {
                      existing_data.label = existing_data.label + '(expired)';
                  }
              }
  ...
  function delete_token (token) {
          var needPopUpConfirmBarMSG = null;
          var token_data = $.data(token.get(0), "tokeninput");
          var onBeforeDelete = settings.onBeforeDelete;
          if($.isFunction(onBeforeDelete)) {
              needPopUpConfirmBarMSG = onBeforeDelete.call(hidden_input,token_data);
              if(needPopUpConfirmBarMSG != null) {
                  initialConfirmBar(needPopUpConfirmBarMSG, real_delete_token, undefined, token);
              } else {
                  real_delete_token(token);
              }
          } else {
              real_delete_token(token);
          }
  ...
  function real_delete_token(token) {

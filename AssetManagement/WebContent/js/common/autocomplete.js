...
// Input box position "enum"
 ...
 // var textTail=null;
  ...
  var token_list = $("<ul />")
      ...
      .mouseout(function (event) {
          ...
          if (value.indexOf(settings.inputSplit) == -1){
        	  ...
        	  if(value!=""  && count==0) {
        		  count =1;
        		  //message="<p>"+inputIllegal()+"</p>";
    	 	      //showError($(".autoText"), message,-170,55,3000);
        	  }
  ...
  function addItems(input) {
      ...
      if (checkedToken.length && settings.autoCheck) {
...
//   if (string && string.indexOf(settings.inputSplit) != -1) {
   if (string) { // Do not need user to end the input with the ";".
   ...
   }else if (string && dropdown.attr('display') == 'none') {
  ...
  function resize_input() {
      ...
      // No need to set width for input
